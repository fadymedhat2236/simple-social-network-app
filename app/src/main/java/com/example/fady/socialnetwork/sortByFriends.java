...
public class sortByFriends extends AppCompatActivity {

    private ArrayList<userField> users;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_sort_by_friends);
        ...
        h1.initialize(c,"numberOfFriends");
        int n=h1.getCount();
        int count=1;
        int oldNumber=-1;
        for(int i=0;i<n;i++)
        {
            ...
            users.add(new userField(node.getName(), "number of friends:" + Integer.toString(node.getNumber()), count));
        }
        UserFieldAdapter usersArray=new UserFieldAdapter(this,users);
        ListView list=findViewById(R.id.friend_field_List);
